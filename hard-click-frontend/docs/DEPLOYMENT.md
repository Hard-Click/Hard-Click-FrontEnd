...
- ⛔ **production 배포는 라이브 상태로** — 가짜 데이터가 뜨지 않게 한다. ⚠️ 단 커밋 기본값은 `USE_MOCK=true`(프리뷰)이고 **라이브 검증된 도메인만 `MOCK_OVERRIDE`에서 `false`로 승격**된다. 전역 하나를 뒤집는 문제가 아니라, `MOCK_OVERRIDE`에 없는 도메인은 전역값(true)을 따라 계속 mock이므로 **배포 대상 도메인이 실연동인지** 확인한다(스위치 구조: `docs/ARCHITECTURE.md` §2 · 인증·공지·커뮤니티는 `USE_MOCK_*` 별도 플래그).
...
- [ ] **mock 게이트 확인** — 배포 도메인이 실연동인지(`USE_MOCK` 전역 + `MOCK_OVERRIDE` 도메인별 + `USE_MOCK_AUTH/NOTICES/COMMUNITY`). 현재 전역 기본값 `true`, 라이브 도메인은 override로 `false` 승격
