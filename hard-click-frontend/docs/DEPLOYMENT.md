...
- ⚠️확인: **어느 브랜치가 Production 배포 트리거인지.** 팀 규칙상 base는 `develop`(`docs/WORKFLOW.md` §1)이지만, Vercel이 어떤 브랜치를 Production으로 물고 있는지는 대시보드 설정 사항이라 코드로 확정 불가.
...
- **`.env.example`은 커밋 O, `.env.local`/`.env.production`은 gitignore** (`docs/WORKFLOW.md` §7). 그래서 **실제 값은 레포에 없다.** 아래 §5 참고.
...
- ⚠️ **활성 라우트의 `page.tsx`가 비어(0바이트) 있으면 `next build`가 실패한다.** 빈 파일은 의도된 스캐폴딩(CLAUDE.md §3)이지만 **배포되는 라우트는 반드시 채워야** 빌드가 통과한다. → PR 올리기 전 로컬에서 `npm run build` 확인(아래 §6 체크리스트).
- ⛔ **production 배포는 라이브 상태로** — 가짜 데이터가 뜨지 않게 한다. ⚠️ 단 커밋 기본값은 `USE_MOCK=true`(프리뷰)이고 **라이브 검증된 도메인만 `MOCK_OVERRIDE`에서 `false`로 승격**된다. 전역 하나를 뒤집는 문제가 아니라, `MOCK_OVERRIDE`에 없는 도메인은 전역값(true)을 따라 계속 mock이므로 **배포 대상 도메인이 실연동인지** 확인한다(스위치 구조: `docs/ARCHITECTURE.md` §2 · 인증·공지·커뮤니티는 `USE_MOCK_*` 별도 플래그). 또한 `FORCE_ALL_MOCK`·`E2E_MOCK`(프리뷰·테스트용)가 켜져 있으면 도메인 override보다 **먼저 전체를 강제 mock**으로 만드므로 배포 전 둘 다 OFF여야 한다.
...
- [ ] **mock 게이트 확인** — 배포 도메인이 실연동인지(`USE_MOCK` 전역 + `MOCK_OVERRIDE` 도메인별 + `USE_MOCK_AUTH/NOTICES/COMMUNITY`, 그리고 `FORCE_ALL_MOCK`·`E2E_MOCK`는 **OFF**여야 함 — 켜지면 override보다 먼저 전체 강제 mock). 현재 전역 기본값 `true`, 라이브 도메인은 override로 `false` 승격
