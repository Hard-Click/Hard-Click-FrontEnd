...
> 파일 위치·컨벤션은 `CLAUDE.md`(§2·§14), 도메인별 연동 현황은 `src/mocks/config.ts` 주석을 정본으로 본다.
...
> ⚠️ **현재 상태:** 이미 작성된 일부 영역(`store/auth.store.ts` 등 초기 CSR 코드)은 이 원칙과 반대(클라이언트 중심)다. 신규 코드는 Server-First로 작성하고, 구코드는 점진 이전한다. (자세한 격차 표는 `CLAUDE.md` §8)
...
- **변경(CUD)이 있는 도메인** → `actions.ts`를 추가하고 `'use server'` + `revalidatePath`를 반드시 넣는다. (`CLAUDE.md` §5의 6단계 파이프라인)
...
| **NotificationProvider** | 서버 `getNotificationsServer()` | SSE 이벤트 → 재조회 + 낙관적 읽음 | 헤더 종이 client여도 **초기 데이터는 서버 props**에서. SSE는 '초기 페칭'이 아니라 '실시간 구독'이라 §12 예외 |
...
- **SSE가 §12(useEffect 데이터 페칭 금지)를 어기지 않는 이유** — EventSource는 브라우저 전용 **스트림 구독**이지 초기 데이터 페칭이 아니다. 초기 데이터는 서버 props에서 오고, SSE는 "이벤트 수신 = 재조회"로만 쓴다. 그래서 **payload shape에 의존하지 않는다**(BE push 형식이 바뀌어도 안 깨짐).
...
- 렌더링·Server Action·인증 원칙: `CLAUDE.md` §4·§5·§6
...
작성한 문서 전체를 위에 출력했습니다. 파일로 저장하지는 않았습니다(지시대로 최종 마크다운만 반환). 저장 위치가 필요하면 절대경로는 `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/docs/ARCHITECTURE.md` 입니다.

검증에 사용한 실제 소스(절대경로):
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/CLAUDE.md` (§4·§5·§6)
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/mocks/config.ts`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/app/api/[...path]/route.ts`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/lib/api.ts` · `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/services/api.ts`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/features/rankings/server.ts`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/app/layout.tsx` + `src/features/{auth/AuthProvider.tsx,notifications/NotificationProvider.tsx,community/MemberStatusProvider.tsx}` · `src/features/auth/session.ts`
