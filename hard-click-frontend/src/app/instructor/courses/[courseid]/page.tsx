...
import { deleteCourse, toggleCourseStatus } from '@/features/instructor/services';
...
export default function InstructorCourseDetailPage() {
  ...
  const handleToggleStatusConfirm = async () => {
    ...
    const res = await toggleCourseStatus(courseId, nextStatus);
    setIsMutating(false);
    if (!res.success) {
    ...
    <div className="min-h-screen bg-[#F0F2F5]">
      ...
      <div className="w-full max-w-[1440px] mx-auto px-[157.5px]">
        ...
        <div className="pt-10 px-8 pb-0 flex flex-col gap-8">
          ...
          {/* ── 히어로 카드 ── */}
          <div className="bg-white border border-[#D5D8DD] relative" style={{ padding: '33px 33px 1px' }}>
              ...
              <div ref={menuRef} className="absolute top-[33px] right-[33px]">
                ...
                {isMenuOpen && (
                  <div className="absolute right-0 mt-1 w-[136px] bg-white border border-[#E2E8F0] rounded-[14px] shadow-md overflow-hidden z-20">
                    ...
                    <button
                      type="button"
                      onClick={() => { setIsMenuOpen(false); setIsStatusModalOpen(true); }}
      ...
      {isStatusModalOpen && (
        <ConfirmModal
          ...
          description={isPublished ? '해당 강의를 비공개로 전환하시겠습니까?' : '해당 강의를 공개로 전환하시겠습니까?'}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      >
        ...
        <h2 className="text-center text-xl font-bold leading-8 text-[#1F2937]">{title}</h2>
        <p className="mt-2 text-center text-sm leading-5 text-[#6B7280]">{description}</p>
