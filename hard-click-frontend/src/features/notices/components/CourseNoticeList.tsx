...
function CourseNoticeCard({
  ...
  return (
    <Link
      href={`${noticeBasePath}/${notice.noticeId}`}
...
/**
 * 강의별 공지 목록 화면 — 표시 전용 Server Component.
 * 학생/강사가 동일 화면을 공유한다. 공지 상세 링크(`noticeBasePath`)·강의로 돌아가기(`backHref`)는
 * 사용처에 따라 주입해 레이아웃(헤더)을 유지한다.
 */
export default function CourseNoticeList({
  courseTitle,
  instructorName,
  notices,
  totalPages,
