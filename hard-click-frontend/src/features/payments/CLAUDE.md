...
- ⛔ **시크릿 키(Secret Key)는 프론트에도 Git에도 절대 두지 않는다.** 시크릿으로 하는 **토스 최종 승인은 백엔드가** 한다(아래 흐름 참조). `NEXT_PUBLIC_` 접두사에 시크릿을 넣는 실수 금지 — 브라우저에 노출된다(루트 §12 환경변수).
...
> 팀 규칙: **결제 의도는 mock 유지 원칙** — 다만 **강의 결제만 실토스 연동 완료**, 나머지는 mock. 아래는 *실제 코드가 지금 무엇을 하는지* 그대로다(문서용 이상론 아님).

- **mock 폴백은 도메인이 아니라 "Client Key 유무"로 갈린다** (`PaymentButton`):
  - `TOSS_CLIENT_KEY`가 세팅됐고 `type==='course'`(+선택분 1개↑) → **실토스** 흐름.
  - 키가 있으면 `type==='subscription'`도 코드상 **실토스 경로(`canTossSub`)** 를 탄다 → confirm이 구독권 지급.
  - **키가 비면(그 외 전부)** → `setTimeout(1200ms)` 후 성공 화면으로 보내는 **가짜-성공 mock**. 이건 처리 흐름만 재현하는 것이므로, 실결제로 오해하지 말 것.
  - ⚠️확인: MEMORY의 "구독은 mock" 서술과 **현재 코드가 다르다**. 코드는 *키가 있으면 구독도 실토스*다. "구독=mock"을 유지하려면 구독 경로를 명시적으로 막아야 하고, 지금은 키 유무에만 의존한다. → 팀에 의도 확인 필요.
...
- **PR 전 `USE_MOCK=false` 원복 필수**(루트 §12/§13). mock 상태로 결제 코드를 올리지 않는다.
...
작성 근거 파일(절대경로):
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/features/payments/toss.ts`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/features/payments/actions.ts`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/features/payments/server.ts`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/features/payments/types.ts`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/features/payments/components/PaymentButton.tsx`
- `/Users/hyun/Hard-Click-FrontEnd/hard-click-frontend/src/app/(user)/payment-result/PaymentResultClient.tsx`
