 ...
 *    study 도메인 URL(/api/studies)이라 곽시윤 커뮤니티와 공유되는 지점.
 */
...
export async function enterStudyChatAction(
  ...
  // ── BE 연동 seam ── GET /api/studies/{groupId} → data.chatRoomId
...
export async function joinStudyChatAction(
  ...
  // ── BE 연동 seam ── POST /api/studies/{groupId}/join → data.chatRoomId
  //   409(정원참)·400(이미참여) 등은 res.message로 그대로 안내.
  const res = await serverApi.post<{
    groupId: number;
    chatRoomId: number;
    currentCount: number;
  }>(`/api/studies/${groupId}/join`);
  if (!res.success || !res.data) {
