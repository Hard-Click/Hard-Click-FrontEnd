...
describe('getCartServer — 이미 수강중(구매 완료)인 강의 제외 (로그인)', () => {
  ...
  function wire(
    ...
    mockGet.mockImplementation((url: string) =>
      url.includes('/enrollments/me')
        ? Promise.resolve(
            // BE는 status=ALL에 status를 항상 내려준다. 여기선 active(수강중)로 고정 —
            // REFUNDED가 '보유중'에서 제외되는지는 별도 회귀 테스트에서 검증.
            okEnvelope(
  ...
  it('환불(REFUNDED)한 강의는 보유중이 아니므로 장바구니에 그대로 남는다', async () => {
    // status=ALL은 환불(REFUNDED)까지 포함 — active(IN_PROGRESS/COMPLETED)만 '보유중'으로 제외한다.
    // (REFUNDED를 '보유중'으로 오판하면 환불한 강의를 다시 담아도 장바구니에서 사라져 재구매가 막힘)
    mockGet.mockImplementation((url: string) =>
      url.includes('/enrollments/me')
        ? Promise.resolve(
            okEnvelope([
              { courseId: 1, status: 'REFUNDED' }, // 환불 → 보유중 아님
              { courseId: 3, status: 'IN_PROGRESS' }, // 수강중 → 제외 대상
            ]),
          )
        : Promise.resolve(
            okEnvelope({
    ...
    // courseId=1(REFUNDED)은 유지, courseId=3(IN_PROGRESS)만 제외
    expect(cart.items).toHaveLength(1);
    expect(cart.items[0].courseId).toBe(1);
    expect(cart.totalPrice).toBe(99000);
    expect(cart.totalCount).toBe(1);
  });
