...
describe('getAdminQuizFormMetaServer — 관리자는 소유자무관 목록으로 takenWeeks 집계', () => {
  afterEach(() => jest.clearAllMocks());

  it('takenWeeks를 /api/admin/quizzes/courses/{id}(WeeklyQuiz)에서 뽑는다 — 강사 엔드포인트(소유자 필터) 아님', async () => {
    // ⚠️ 강사 /api/instructor/quizzes는 로그인 관리자 소유 퀴즈만 반환(0개)이라 중복차단이 깨진다.
    //    관리자 목록 /api/admin/quizzes/courses/{id}는 강의 전체(소유자무관)라 실제 점유 주차를 집계한다.
    mockGet.mockImplementation((url: string) =>
      Promise.resolve(
        url.includes('/api/courses/')
          ? ok({
              sections: [
                { sectionId: 1, orderIndex: 1, title: '1주차' },
                { sectionId: 2, orderIndex: 2, title: '2주차' },
              ],
            })
          : ok({
              courseId: 1,
              weeks: [
                {
                  quizId: 9,
                  weekNumber: 2, // 2주차 점유(다른 강사가 만든 퀴즈여도 관리자 목록엔 잡힘)
                  quizTitle: 'q',
                  status: 'PUBLISHED',
                  totalQuestionCount: 3,
                  examDate: '2026-07-10T00:00:00Z',
                },
              ],
            }),
      ),
    ...
    const meta = await getAdminQuizFormMetaServer(1);
    expect(meta.weeks).toEqual([1, 2]);
    expect(meta.takenWeeks).toEqual([2]); // 관리자 목록 weekNumber 2 → 2주차 점유
    // 관리자 엔드포인트로 조회하고, 강사 엔드포인트는 쓰지 않는다
    expect(mockGet).toHaveBeenCalledWith('/api/admin/quizzes/courses/1');
    expect(mockGet).not.toHaveBeenCalledWith(
      expect.stringContaining('/api/instructor/quizzes'),
    );
  });
});
...
describe('getInstructorQuizDetailServer — 상세 매퍼 (정답 인덱스 복원)', () => {
  ...
  it('난이도 프리필 — 기존 null 문항은 중(2) 기본, 값 있으면 그대로', async () => {
    mockGet.mockResolvedValue(
      ok({
        quizId: 5,
        quizTitle: 'q',
        ...
        questions: [
          {
            ...
            questionText: 'Q1',
            explanation: '',
            difficulty: null, // 난이도 도입 이전 문항 → 중(2)으로 기본 프리필
            correctOptionId: 11,
            options: [{ optionId: 11, optionText: 'a', correct: true }],
          },
          {
            questionId: 2,
            questionText: 'Q2',
            explanation: '',
            difficulty: 3, // 값 있으면 그대로
            correctOptionId: 21,
            options: [{ optionId: 21, optionText: 'a', correct: true }],
          },
        ],
      }),
    );
    ...
    const quiz = await getInstructorQuizDetailServer(5);
    expect(quiz?.questions[0].difficulty).toBe(2); // null → 중
    expect(quiz?.questions[1].difficulty).toBe(3); // 유지
  });
});
...
describe('getQuizScoresServer — 통계 주차는 BE weekNumber (제목 정규식 X)', () => {
  afterEach(() => jest.clearAllMocks());

  it('제목 숫자와 어긋나도 weekNumber를 그대로 쓴다(정규식이면 5가 될 제목)', async () => {
    // BE가 통계 응답에 weekNumber를 추가(2026-07-13) → 목록·리포트와 동일 스킴. 제목파싱 폐기 검증.
    mockGet.mockResolvedValue(
      ok({
        courseTitle: 'c',
        weekNumber: 2, // 섹션 orderIndex 기반
        sectionTitle: '5단원. 함수', // 제목숫자5 → 정규식이면 5(불일치)
        quizTitle: 'q',
        summary: { submittedCount: 1, notSubmittedCount: 0, averageScore: 90 },
        scoreDistribution: [],
        students: [],
      }),
    );

    const board = await getQuizScoresServer(1, 9);
    expect(board?.week).toBe(2); // weekNumber(2)지 제목파싱(5)이 아니다 → 목록↔통계 정합
  });

  it('관리자 통계(getAdminQuizScoresServer)도 weekNumber를 그대로 쓴다', async () => {
    // 강사·관리자 통계 둘 다 전환됐는지 — 관리자 경로 회귀 방지.
    mockGet.mockResolvedValue(
      ok({
        courseTitle: 'c',
        weekNumber: 3,
        sectionTitle: '섹션 #9373', // 삭제섹션 → 정규식이면 9373(불일치)
        quizTitle: 'q',
        summary: { submittedCount: 0, notSubmittedCount: 0, averageScore: 0 },
        scoreDistribution: [],
        students: [],
      }),
    );

    const board = await getAdminQuizScoresServer(1, 9);
    expect(board?.week).toBe(3); // weekNumber(3)지 제목파싱(9373)이 아니다
  });
});
