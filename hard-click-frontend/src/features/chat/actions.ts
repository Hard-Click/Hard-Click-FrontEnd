 ...
 *   - 나가기: DELETE /api/study/{groupId}/leave (방장은 참여자 남으면 403)
...
export async function leaveStudyChatAction(
  ...
  // ── BE 연동 seam ── DELETE /api/study/{groupId}/leave (방장은 참여자 남으면 403 → res.message 안내)
  const res = await serverApi.delete(`/api/study/${groupId}/leave`);
  if (!res.success) {
