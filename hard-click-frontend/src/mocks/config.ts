...
const MOCK_OVERRIDE: Record<string, boolean> = {
  ...
  onboarding: false, // /api/onboarding/{me,profile,availability,exam-scores} — 라이브(#917). ALB+실 학생 계정 토큰으로 4종 순차 검증(2026-07-18, 상세는 #929): PUT profile→availability→exam-scores 저장할 때마다 GET /me의 completed 플래그가 순서대로 true로 바뀌고, 3개 다 채우면 onboarded:true. dailyCapMin도 가용시간에서 정상 유도(480). ⚠️ 인프라 이전 전 폐기 대기 중인 구버전 단일서버로 검증하면 온보딩 스키마가 없어 전부 500 — 반드시 ALB로 확인할 것. BE 미배포/에러 시 되돌리려면 이 값 true.
