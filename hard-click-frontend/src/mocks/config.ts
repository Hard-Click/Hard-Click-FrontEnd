...
const MOCK_OVERRIDE: Record<string, boolean> = {
  ...
  churn: false, // /api/admin/churn/{summary,trend,reasons,students,students/{id}} — 이탈관리 대시보드 라이브. ALB 검증(2026-07-17, admin_demo 토큰): 4종 + 목록 파라미터 조합(미지정/HIGH/MEDIUM/page=1/빈level) + 상세(enr 2~5) 전부 200. ⚠️ 실데이터 위험군은 2명(정위험 HIGH·최밀림 MEDIUM)뿐이라 mock 8명처럼 안 채워짐 — 박모범·이눈치는 risk<0.4라 목록 제외가 정상. 되돌리려면 이 값 true.
  schedule: false, // /api/schedule/me·/me/today — 학습 스케줄/오늘 할 일 라이브(2026-07-16). ALB+페르소나 로그인 E2E 검증 완료(2026-07-17: p_model 4건·p_atrisk 7건 200). BE 미배포/에러 시 스케줄 화면이 에러남 → 되돌리려면 이 값 true.
  similarQuiz: false, // POST /api/similar-quizzes — 오답 기반 유사문제 라이브(2026-07-16). ALB+페르소나 로그인 검증 완료(2026-07-17: 200, courseId 요청 shape 일치). ⚠️ 오답/유사문제 없으면 data=null(정상 empty) — non-null 응답 shape(questions[].options 등)는 아직 미대조. BE 미준비 시 유사퀴즈 진입이 에러남 → 되돌리려면 이 값 true.
  ...
  onboarding: false, // /api/onboarding/{me,profile,availability,exam-scores} — 라이브(#917). ALB+실 학생 계정(geunji04) 토큰으로 4종 순차 검증(2026-07-18): PUT profile→availability→exam-scores 저장할 때마다 GET /me의 completed 플래그가 순서대로 true로 바뀌고, 3개 다 채우면 onboarded:true. dailyCapMin도 가용시간에서 정상 유도(480). ⚠️ 구버전 단일서버(13.125.94.217, 폐기 대기)로 검증하면 온보딩 스키마가 없어 전부 500 — 반드시 ALB로 확인할 것. BE 미배포/에러 시 되돌리려면 이 값 true.
