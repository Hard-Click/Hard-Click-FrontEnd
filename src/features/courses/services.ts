...
const MOCK_COURSE_DETAIL: CourseDetail = {
  ...
  notices: [
    {
      noticeId: 1,
      title: '6월 모의고사 대비 특별 추가 강의 업로드 안내',
      content:
        '6월 모의고사 대비 특별 추가 강의가 업로드되었습니다. 수강생 여러분께서는 강의 목록에서 확인하세요.',
      createdAt: '2026-05-20',
      isPinned: true,
    },
    {
      noticeId: 2,
      title: '5월 오답풀이 라이브 일정 변경 안내',
      content:
        '5월 오답풀이 라이브 강의 일정이 5월 28일로 변경되었습니다. 참고 부탁드립니다.',
      createdAt: '2026-05-10',
    },
    {
      noticeId: 4,
      title: '5월 특별 할인 이벤트 안내',
      content: '5월 한 달 동안 모든 강의 30% 할인 진행 중입니다!',
      createdAt: '2026-05-01',
    },
    {
      noticeId: 3,
      title: '강의 자료 v2.1 업데이트 완료',
      content:
        '강의 자료 PDF가 v2.1로 업데이트되었습니다. 마이페이지 자료실에서 다시 다운로드해 주세요.',
      createdAt: '2026-04-28',
    },
    {
      noticeId: 5,
      title: 'Q&A 게시판 운영 안내',
      content: '궁금한 점은 Q&A 게시판에 남겨주시면 24시간 내 답변드립니다.',
...
export async function getCourseDetail(
  ...
  if (USE_MOCK) {
    ...
    if (courseId === 4)
      return { ...MOCK_COURSE_DETAIL, courseId: 4, status: 'DELETED' };
    if (courseId === 5)
      return { ...MOCK_COURSE_DETAIL, courseId: 5, status: 'HIDDEN' };
    if (courseId === 6)
      return {
        ...MOCK_COURSE_DETAIL,
        courseId: 6,
    ...
    return {
      ...MOCK_COURSE_DETAIL,
      ...item,
      description: MOCK_COURSE_DETAIL.description,
    };
  }
...
export async function getCourses(
  ...
  if (USE_MOCK) {
    ...
    if (query?.subjectId) {
      ...
      if (subject)
        courses = courses.filter((c) => c.subjectName === subject.name);
    }
...
export function getInstructors(): string[] {
  return Array.from(
    new Set(
      MOCK_COURSES.filter((c) => c.status === 'PUBLISHED').map(
        (c) => c.instructorName,
      ),
    ),
  ).sort();
